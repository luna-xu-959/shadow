...
const MultiplayerMenuScene := preload(GamePaths.MULTIPLAYER_MENU_SCENE)
const PauseMenuScene := preload(GamePaths.PAUSE_MENU_SCENE)
const NetworkScript := preload(GamePaths.NETWORK_MANAGER)
...
func _return_to_main_menu() -> void:
	...
	if _game_manager.has_method("reset_match"):
		_game_manager.call("reset_match")
	get_tree().paused = true
...
func _on_network_session_started(is_host: bool) -> void:
	...
	if is_host:
		...
		_finish_session_start(
			(
				"Hosting up to %d players (Team A vs Team B, 1:1).\n"
				+ "Tailscale IP: %s | Esc = pause"
			) % [SessionConfig.MAX_PLAYERS, _get_tailscale_ip_hint()]
		)
	else:
		_configure_online_client()
		_finish_session_start("Joined match. Stomp enemy shadows. Esc = pause.")
...
func _finish_session_start(message: String) -> void:
	get_tree().paused = false
	_menu.hide_menu()
	if _game_manager.has_method("start_match"):
		_game_manager.call("start_match")
	if _status_label:
...
func _configure_local_split() -> void:
	...
	_player0.set_team(TeamInfo.Id.TEAM_A)
	_player1.set_team(TeamInfo.Id.TEAM_B)
	...
	_finish_session_start("Local 2v0 test — Team A (P0) vs Team B (P1). Esc = pause.")
