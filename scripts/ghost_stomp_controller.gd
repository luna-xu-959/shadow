...
signal stomp_hit(human: Node, method: String)
...
func _ready() -> void:
	_ghost = get_parent() as CharacterBody3D
	if _ghost == null:
		set_process(false)
		return
	_scene_root = _ghost.get_parent()
	_game_manager = _scene_root.get_node_or_null("GameManager")
	_sun_system = _scene_root.get_node_or_null("SunSystem")
	await get_tree().process_frame
	_human = _scene_root.get_node_or_null("Player0") as CharacterBody3D
	if _human:
		_human_presence = _human.get_node_or_null("HumanPresence")
		if _human_presence and _human_presence.has_signal("escaped_charge"):
			if not _human_presence.escaped_charge.is_connected(_on_human_escaped):
				_human_presence.escaped_charge.connect(_on_human_escaped)
	_build_fx()
	_cache_feet()


func _cache_feet() -> void:
	var visual := _ghost.get_node_or_null("Visual")
	if visual == null:
		return
...
func is_in_light_zone() -> bool:
	if _scene_root == null or _ghost == null:
		return true
	return ShadowRules.is_light_zone(_scene_root, _ghost.global_position)
...
func _ensure_human_presence() -> void:
	if _human == null:
		_human = _scene_root.get_node_or_null("Player0") as CharacterBody3D
	if _human_presence == null and _human:
		_human_presence = _human.get_node_or_null("HumanPresence")
		if _human_presence and _human_presence.has_signal("escaped_charge"):
			if not _human_presence.escaped_charge.is_connected(_on_human_escaped):
				_human_presence.escaped_charge.connect(_on_human_escaped)
...
func _sync_attack_input_edges() -> void:
	...
	_attack_key_held = InputMap.has_action("p1_attack") and Input.is_action_pressed("p1_attack")
...
func _attack_just_pressed() -> bool:
	if InputMap.has_action("p1_attack") and Input.is_action_just_pressed("p1_attack"):
		return true
	return _mouse_lmb_held and not _mouse_lmb_was_held


func _attack_just_released() -> bool:
	if InputMap.has_action("p1_attack") and Input.is_action_just_released("p1_attack"):
		return true
...
func _physics_process(delta: float) -> void:
	if _ghost == null:
		return
	if multiplayer.has_multiplayer_peer() and not _ghost.is_multiplayer_authority():
		return
	_sync_attack_input_edges()
	_ensure_human_presence()
	match _phase:
		ChargePhase.IDLE:
			_handle_idle(delta)
...
func _handle_attack_input() -> void:
	if _game_manager and _game_manager.has_method("is_game_finished") and _game_manager.call("is_game_finished"):
		return
	_ensure_human_presence()
	if _human_presence and _human_presence.has_method("allows_instant_stomp") and _human_presence.allows_instant_stomp():
...
func _has_attackable_core() -> bool:
	if _human_presence == null or not _human_presence.has_method("is_core_exposed"):
		return false
	if not _human_presence.is_core_exposed():
		return false
	if not _human_presence.has_method("get_lifecycle"):
		return false
	var lifecycle: int = _human_presence.get_lifecycle()
	return (
...
func _get_core() -> ShadowCore:
	if _human_presence == null or not _human_presence.has_method("get_shadow_core"):
		return null
	return _human_presence.get_shadow_core() as ShadowCore
...
func _update_charge(delta: float) -> void:
	...
	if not _attack_held():
		_cancel_charge(false)
		return
	if _human_presence and _has_chargeable_core_in_range():
		_human_presence.notify_escape_attempt()
	_charge_time += delta
...
func _right_foot_position() -> Vector3:
	...
	return _ghost.global_position + Vector3(0.11, 0.15, 0.0)
...
func _left_foot_anchor() -> Vector3:
	...
	var body := _ghost.global_position
